...
public class ReadThread extends Thread {
    ...
    public void run() {
        if (tipoSocket == 0) {
            GestoreSocketComandi.responseLine = null;
        } else {
        ...
        if (tipoSocket == 0) {
            if (GestoreSocketComandi.timeoutThread != null) {
                GestoreSocketComandi.timeoutThread.interrupt();
            }
        } else {
            if (GestoreSocketMonitor.timeoutThreadMon != null) {
                GestoreSocketMonitor.timeoutThreadMon.interrupt();
            }
        }
