...
public class AuthometionShield extends Protocol {

    private static final Logger LOG = Logger.getLogger(AuthometionShield.class.getName());
    ...
    private AuthometionShieldGui gui;

    public AuthometionShield() {
        super("Authometion Shield", "/authometion-shield/authometion-shield-manifest.xml");
        setPollingWait(-1); // onRun() executes once.
    }

    @Override
    public void onStart() throws PluginStartupException {
        gui = new AuthometionShieldGui(this);
        try {
            serial = new SerialHelper(portName, baudRate, dataBits, stopBits, parity, new SerialPortListener() {
                @Override
                public void onDataAvailable(String data) {
                    LOG.log(Level.CONFIG, "Authometion Shield received: {0}", data);
                    sendChanges(data);
                }
            });
            ...
            throw new PluginStartupException("Error while creating Authometion Shield serial connection. " + ex.getMessage(), ex);
        }
    }
    ...
    private void sendChanges(String data) {
        ...
        if (data.startsWith("STATUS:")) {
            ...
            ProtocolRead event = new ProtocolRead(this, "authometion-shield", payload[0]);
    ...
    public void writeToSerial(String message) throws UnableToExecuteException {
        try {
            ...
            throw new UnableToExecuteException("Error writing message '" + message + "' to Authometion Shield: " + ex.getMessage(), ex);
        }
    }

    private void initializeThings() throws UnableToExecuteException, InterruptedException {
        for (EnvObjectLogic object : getApi().things().findByProtocol("authometion-shield")) {
